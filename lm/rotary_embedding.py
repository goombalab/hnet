...
from torch import Tensor, arange, autograd, cos, float32, outer, sin
...
class RotaryEmbedding(Module):
  ...
  def __init__(
    ...
    scale = (
      (arange(0, dim, 2, device=device, dtype=float32) + 0.4 * dim)
      / (1.4 * dim)
  ...
  def _update_cos_sin_cache(self, seqlen, device=None, dtype=None):
    ...
    if (
      ...
      if self.pos_idx_in_fp32:
        t = arange(seqlen, device=device, dtype=float32)
        # We want fp32 here as well since inv_freq will be multiplied with t, and the output
        # will be large. Having it in bf16 will lose a lot of precision and cause the
        # cos & sin output to change significantly.
        # We want to recompute self.inv_freq if it was not loaded in fp32
        if self.inv_freq.dtype != float32:
          inv_freq = self._compute_inv_freq(device=device)
        else:
          inv_freq = self.inv_freq
      else:
        t = arange(seqlen, device=device, dtype=self.inv_freq.dtype)
        inv_freq = self.inv_freq
      # Don't do einsum, it converts fp32 to fp16 under AMP
      # freqs = torch.einsum("i,j->ij", t, self.inv_freq)
      freqs = outer(t, inv_freq)
      if self.scale is None:
        self._cos_cached = cos(freqs).to(dtype)
        self._sin_cached = sin(freqs).to(dtype)
      else:
        power = (
          arange(seqlen, dtype=self.scale.dtype, device=self.scale.device)
          - seqlen // 2
        ) / self.scale_base
        ...
        self._cos_cached = (cos(freqs) * scale).to(dtype)
        self._sin_cached = (sin(freqs) * scale).to(dtype)
        self._cos_k_cached = (cos(freqs) / scale).to(dtype)
        self._sin_k_cached = (sin(freqs) / scale).to(dtype)
